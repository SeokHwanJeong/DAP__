...
for link_data in link_datas:
    ...
    for movie_link in link_data:
        ...
        try:#관람객 평점 있음
            visitors = driver.find_element(By.XPATH, '//*[@id="actualPointPersentBasic"]/div')
            test = visitors.find_element(By.TAG_NAME, "em") #점수가 없으면 오류남
            visitors_depth1 = visitors.find_elements(By.TAG_NAME, "em")
            temp = ""
            for i in visitors_depth1:
                temp += i.text
            visitors_score = float(temp)
        except:#관람객 평점 없음
            visitors_score = np.NaN
        ...
        # 관람객 참여 수
        # 태그에서 display = none으로 되어있으면 택스트를 가져올 수 없어서 바꿔줬음
        try:
            ...
            visitors_count = driver.find_element(By.XPATH, '//*[@id="actualPointCountBasic"]/em').text
            visitors_count = int(visitors_count.replace(',',''))
        except:
            visitors_count = 0

        # 평론가 참여 수
        # XPATH가 동영상칸이 없으면 바뀌는 영화들이 있음
        try:
            special_score = driver.find_element(By.CLASS_NAME, 'special_score')
            c_user_count = special_score.find_element(By.CLASS_NAME, 'user_count')
            critic_count = c_user_count.find_element(By.TAG_NAME, 'em').text
        ...
        try:
            movie_temp = [movie_name,  year, visitors_score, critic_score, netizens_score, visitors_count, critic_count, netizens_count]
            print(movie_temp)
